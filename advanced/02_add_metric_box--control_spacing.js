...
var components = {
       ...
       // selectedFont: { label: 'search for', value: getSuggestion(document.activeSymbol) },
        setLeftSidebearing: { label: 'Set Left Sidebearings', value: true },
        addWidth_left: { label: 'Value', value: 60 },
        setRightSidebearing: { label: 'Set Right Sidebearings', value: true },
        addWidth_right: { label: 'Value', value: 60 }
			};
...
print("setBox started //////////////");
    ...
    for(var i = 0; i < document.symbols.length; i++) {
        ...
        //////////////////
         ...
         ///get Box - Symbol
         if(!allLetterSymbols[values.boxName]) {Dialog.alert("There was no -" + values.boxName + "- found. Please add a rectangle as symbol, so we can use it as a metric body box. (should be a bit larger in height than any drawn character)");};
...
document.symbols[i].definition.children['box'].translate(document.symbols[i].definition.children['Letter'].position);
    ...
    //document.symbols[i].definition.placedSymbol.translate(new Point(0,0))
       ...
       // document.symbols[i].appendTop(placedSymbol);
        ...
        var previousSidebearing_Left = Math.round(1000 / mybox.controlBounds.height * (myLetter.strokeBounds.leftCenter.x - mybox.strokeBounds.leftCenter.x));  
        
              
        var previousSidebearing_Right = Math.round(1000 / mybox.controlBounds.height * (mybox.strokeBounds.rightCenter.x - myLetter.strokeBounds.rightCenter.x)); 
               
          print("previousSidebearing_Left" + previousSidebearing_Left);     
          print("previousSidebearing_Right" + previousSidebearing_Right);           
                
              
                
              if((values.setRightSidebearing || values.setLeftSidebearing) && mybox) {
              	
              	 if(!values.setRightSidebearing) {
              	 	values.addWidth_right = previousSidebearing_Right;
              	 	};
              	 	
              	 if(!values.setLeftSidebearing) {
              	 	values.addWidth_left = previousSidebearing_Left;
              	 	};
